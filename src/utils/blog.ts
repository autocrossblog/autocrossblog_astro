...
export async function getRelatedPosts(originalPost: Post, maxResults: number = 4): Promise<Post[]> {
  ...
  const originalTagsSet = new Set(originalPost.tags?.map((tag) => tag.slug) || []);
  const postsWithScores = allPosts
    .filter((post) => post.slug !== originalPost.slug)
    .map((post) => ({
      post,
      score: (post.category?.slug === originalPost.category?.slug ? 5 : 0) +
        (post.tags?.filter((tag) => originalTagsSet.has(tag.slug)).length || 0),
    }));
